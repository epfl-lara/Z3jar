...
class SatSolver extends Z3JarSuite {
  ...
  test("Sat solver") {
    ...
    def DPLL(f : Set[Clause]) : (Boolean, Map[String,Option[Boolean]]) = 
      if(f.isEmpty) (true, Map.empty)
        ...
        val litMap = scala.collection.mutable.Map.empty[String,BoolExpr]
        
        literals.foreach(lit => {
          if(!litMap.keySet.contains(lit.name)) {
            val ast = z3.mkBoolConst(z3.mkSymbol(lit.name))
            litMap(lit.name) = ast
          }
        })
        ...
        f.foreach(clause => {
          if (clause.size > 1) {
            val nc: Array[BoolExpr] = new Array[BoolExpr](clause.size)
            var c: Int = 0
            clause.foreach(lit => {
              nc(c) = if(lit.polarity) litMap(lit.name) else z3.mkNot(litMap(lit.name))
              c = c + 1
            })
            solver.add(z3.mkOr(nc : _*))
          } else {
            val singleLit = clause.head
            if(singleLit.polarity)
              solver.add(litMap(singleLit.name))
            else
              solver.add(z3.mkNot(litMap(singleLit.name))) 
          }
        })
        ...
        result match {
          ...
          case Some(true) => (true, Map.empty[String,Option[Boolean]] ++ litMap.map {
            case (name, sym) => name -> Option(model.eval(sym, false)).map(_.asInstanceOf[BoolExpr].isTrue)
          })
        }
      }
